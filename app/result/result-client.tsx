    ...
    return (
      <main className="grid min-h-screen place-items-center px-4">
        <section className="arena-card max-w-md rounded-lg p-6 text-center">
          <h1 className="hero-title text-3xl font-black">No result found</h1>
          <p className="mt-2 font-semibold text-slate-600">Start a session to generate your score and rank.</p>
          <Link href="/" className="primary-action mt-5 inline-flex min-h-12 items-center justify-center rounded-md px-5 font-bold text-white">
            Go Home
          </Link>
        </section>
      </main>
  ...
  return (
    <main className="mx-auto grid min-h-screen w-full max-w-5xl gap-6 px-4 py-6 md:grid-cols-[1fr_360px] md:py-10">
      <section className="arena-card flex flex-col justify-between rounded-lg p-6 md:p-8">
        <div>
          <div className="liquid-tile mb-8 inline-flex h-16 w-16 items-center justify-center rounded-full text-amber">
            <Award size={34} />
          </div>
          <p className="text-sm font-bold uppercase tracking-wide text-mint">Session Complete</p>
          <h1 className="hero-title mt-2 text-5xl font-black leading-none md:text-7xl">
        ...
        <div className="mt-8 flex flex-wrap gap-3">
          <Link href="/" className="primary-action inline-flex min-h-12 items-center gap-2 rounded-md px-5 font-bold text-white transition hover:-translate-y-0.5">
            <Home size={18} /> Home
          </Link>
          <Link href="/flashcards" className="secondary-action inline-flex min-h-12 items-center gap-2 rounded-md px-5 font-bold text-slate-700 transition hover:-translate-y-0.5">
      ...
      <aside className="arena-card rounded-lg p-5">
        ...
        <div className="grid gap-2">
          {leaderboard.length ? leaderboard.map((row, index) => (
            <div key={`${row.user_id}-${index}`} className={`liquid-tile grid grid-cols-[32px_1fr_auto] items-center gap-3 rounded-md px-3 py-2 ${row.user_id === session.user_id ? "ring-2 ring-mint/45" : ""}`}>
            ...
            <div className="liquid-tile rounded-md border-dashed p-5 text-center text-sm font-semibold text-slate-500">
